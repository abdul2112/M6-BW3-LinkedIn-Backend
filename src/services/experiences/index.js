...
experiencesRouter.get('/:userName/experiences', async (req, res, next) => {
  try {
    const dbResponse = await ExperiencesSchema.find({
...
experiencesRouter.put(
  ...
  async (req, res, next) => {
    try {
      ...
      if (dbResponse1.length > 0) {
        const dbResponse2 = await ExperiencesSchema.findByIdAndUpdate(
          req.params.expId,
          req.body,
...
experiencesRouter.delete(
  ...
  async (req, res, next) => {
    try {
      ...
      if (dbResponse1.length > 0) {
        const dbResponse = await ExperiencesSchema.findByIdAndDelete(
          req.params.expId
        );
        if (dbResponse) {
